...
export default class CategoriaService {
	...
	async updateEmpresa(empresaId: string, body: any, file?: Express.Multer.File) {
		const {
			nome,
			senha,
			email,
		...
		if (file) {
			...
			const newImagemUrl = ImageManager.GenerateImageFileAndUrl(empresa.nome, file.buffer);
